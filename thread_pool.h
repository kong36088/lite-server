...
template <class T> void thread_pool<T>::start() {
...
template <class T> bool thread_pool<T>::append_task(T *task) {
...
template <class T> void *thread_pool<T>::worker(void *argv) {
  thread_pool<task> *pool = (thread_pool<task> *)argv;
  pool->run();
  return pool;
}

template <class T> T *thread_pool<T>::get_task() {
  T *task = NULL;
  queue_mutex_locker.lock();
  if (!task_queue.empty()) {
...
template <class T> void thread_pool<T>::run() {
  while (!is_stop) {
    T *task = get_task();
    if (task == NULL) {
